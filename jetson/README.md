...
The thesis application was validated on a Jetson Orin Nano Super 8GB running Ubuntu 22.04 and Python 3.10.12. It remains the synchronous reference implementation. The Phase 1 package covers host-safe task identity, bounded ownership, a single-worker observable executor and a software periodic probe. Its motion-disabled simulation and fixed-input VLM, ASR and LLM correctness pilots close G5. G6 v2 and v3 are permanently closed after non-replaceable VLM Qwen timeouts. Separate diagnostics and target validation then established the deterministic request, positive unload confirmation and 60 s Qwen boundary frozen in G6 v4. The v4 formal comparison completed all five sessions and 180 measured runs on the Jetson. Every run Gate, lifecycle criterion and responsiveness endpoint passed, but ASR, LLM and VLM each failed to establish the frozen 10% workload-performance noninferiority margin. The overall G6 decision therefore failed. V4 is closed without rerun or threshold change; Phase 1 is closed without meeting its success Gate, and application integration remains unauthorized.

> 中文简介：本目录包含“章鱼号”的 Jetson 端运行程序，负责离线语音交互、本地大模型与视觉模型调用、颜色目标接近和 STM32 串口通信。当前整机应用仍使用已验证的同步路径；Phase 1 已完成固定输入 VLM、ASR、LLM 的 Jetson correctness pilot，G5 已关闭。G6 v2 与 v3 因不可替换的 VLM Qwen 超时而永久关闭，后续诊断和目标机验证支持了 v4 冻结的确定性请求、显式卸载确认与 60 秒 Qwen 边界。G6 v4 已在 Jetson 上完成 5 个 session 和 180 次正式测量；全部运行 Gate、生命周期判据与响应性判据通过，但三个工作负载均未证明冻结的 10% 性能非劣效界限，因此整体 G6 判定失败。v4 已关闭且不重跑或改变阈值；Phase 1 已在未满足成功 Gate 的结论下关闭，整机应用接入仍未获授权。
...
These limitations define the preserved baseline for the separate
residency-aware resource-management study described in the
