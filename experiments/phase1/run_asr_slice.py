"""Run one fixed-input, motion-disabled Phase 1 ASR condition on Jetson."""
...
def run_once(
    ...
    *,
    ...
    """Execute one preflighted ASR request and independently validate it."""
    ...
    condition = ASRSliceCondition(args.condition)
    if args.adapter_execution_timeout_s >= args.completion_timeout_s:
        raise ASRRunError(
    ...
    resolved_preflight_builder = preflight_builder or build_asr_preflight
    resolved_sampler_factory = sampler_factory or TegrastatsSampler
    safety = require_motion_disabled()
    payload = fixed_asr_payload(args.input)
    preflight = resolved_preflight_builder(
        root,
        input_payload=payload,
        expected_branch="main",
    )
    failed_checks = asr_preflight_errors(preflight)
    if failed_checks:
        raise ASRRunError("ASR preflight failed: " + "; ".join(failed_checks))

    output_root = _resolve_output_root(Path(args.output_root), root)
    run_id = make_asr_run_id(condition, args.repetition)
    run_dir = output_root / session_id / condition.value / run_id
    if run_dir.exists():
        raise FileExistsError(f"refusing to overwrite ASR run: {run_dir}")
    ...
    spec_record["adapter_execution_timeout_s"] = args.adapter_execution_timeout_s
    environment = collect_environment(root)
    manifest: dict[str, object] = {
        "manifest_schema_version": MANIFEST_SCHEMA_VERSION,
        "event_schema_version": SCHEMA_VERSION,
        "artifact_kind": "phase1_fixed_input_asr_run",
        "run_id": run_id,
        "session_id": session_id,
        "condition": condition.value,
        "adapter_isolation": "whisper_subprocess",
    ...
    try:
        ...
        adapter = (
            ...
            else FixedInputASRAdapter(
                execution_timeout_s=args.adapter_execution_timeout_s,
                poll_interval_s=args.adapter_poll_interval_s,
                terminate_timeout_s=args.adapter_terminate_timeout_s,
                kill_timeout_s=args.adapter_kill_timeout_s,
            )
        )
        ...
        if not sampler_report.successful:
            raise ASRRunError("tegrastats did not produce a valid closed trace")
        ...
        if summary.get("valid") is not True:
            raise ASRRunError("one or more ASR slice Gates failed")
        ...
        if validation_errors:
            raise ASRRunError(
                "final ASR validation failed: " + "; ".join(validation_errors)
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--condition",
        choices=[condition.value for condition in ASRSliceCondition],
        required=True,
    )
    ...
    parser.add_argument("--adapter-execution-timeout-s", type=float, default=120.0)
    parser.add_argument("--adapter-poll-interval-s", type=float, default=0.05)
    parser.add_argument("--adapter-terminate-timeout-s", type=float, default=2.0)
    parser.add_argument("--adapter-kill-timeout-s", type=float, default=2.0)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Phase 1 ASR run failed: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
    print(run_dir)
    return 0
